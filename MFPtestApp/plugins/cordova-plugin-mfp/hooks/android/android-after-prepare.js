...
function AndroidAfterPrepare(projectDirectory) {
    ...
    */
    ...
    function modifyWorklightJS() {
        ...
        logSilly('Deleting worklight.js');
        that.deleteFile(hookConsts.WORKLIGHT_PATH_ANDROID);
        ...
        that.copyFile(path.join(projDir,
            ...
            hookConsts.PREVIEW_IBMMFPF_PATH_ANDROID));
    }
    ...
    function modifyBootstrapJS() {
        ...
        that.copyFile(path.join(projDir,
            ...
            hookConsts.BOOTSTRAP_PATH_ANDROID));
    }

    /*
    Adds node_modules that are dependencies for ibmmfpf-preview.js

    An error is thrown if modules cannot be copied.
     */
    function addNodeModules() {
        logVerbose('Adding node_modules for ibmmfpf-preview.js.');

        // Copy node_modules for ibmmfpf-preview.js
        var modules = hookConsts.IBMMFPF_NODE_MODULES;
        modules.forEach(function(module) {
            that.copyDir(path.join(projDir, hookConsts.MFP_PLUGIN_DIR, module),
                path.join(projDir, hookConsts.WORKLIGHT_DIR_ANDROID, 'node_modules'));
        });
    }
